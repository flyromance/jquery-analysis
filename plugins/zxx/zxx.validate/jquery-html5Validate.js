...
(function ($, undefined) {
  ...
  $.testRemind = (function () {
    ...
    var fnMouseDown = function (e) {
        ...
        if (
          e.target.id !== $.testRemind.id &&
          $(e.target).parents("#" + $.testRemind.id).length === 0
        ) {
          $.testRemind.hide();
        }
      },
  ...
  $.html5Validate = (function () {
    ...
    return {
      ...
      isRegex: function (ele, regex, params) {
        ...
        regex =
          ...
          (function () {
            ...
            return (
              type &&
              $.map(type.split("|"), function (typeSplit) {
      ...
      isOverflow: function (ele) {
        ...
        if (!attrMin && !attrMax) {
          (attrDataMin = $(ele).attr("data-min")),
            (attrDataMax = $(ele).attr("data-max"));
          ...
          if (attrMin && value < attrMin) {
            ...
            attrStep &&
            !/^\d+$/.test(Math.abs(value - attrMin || 0) / attrStep)
          ) {
            $(ele).testRemind("值无效");
          } else {
            return false;
      ...
      isAllpass: function (elements, options) {
        ...
        if (
          elements.size &&
          elements.size() == 1 &&
          elements.get(0).tagName.toLowerCase() == "form"
        ) {
          elements = elements.find(":input");
        } else if (
          elements.tagName &&
          elements.tagName.toLowerCase() == "form"
        ) {
          elements = $(elements).find(":input");
        }
        var self = this;
        ...
        elements.each(function () {
          ...
          if (type) {
            ...
            if (
              !params.hasTypeNormally &&
              $.html5Validate.isSupport &&
              type != typeReplace
            ) {
          ...
          if (type == "radio" && isRequired) {
            ...
            (isRequired && self.isEmpty(el)) ||
            !(allpass = self.isRegex(el))
          ) {
            // 各种类型文本框以及文本域
  ...
  $.fn.extend({
    ...
    testRemind: function (content, options) {
      ...
      this.remind = $(
        '<div id="' + $.testRemind.id + '">' + content + "</div>"
      ).css(params.css);
      ...
      if (
        ...
        (maxWidth = parseInt(params.css.maxWidth)) &&
      ...
      var fnCreateCorner = function (beforeOrAfter) {
        ...
        if (beforeOrAfter === "before") {
          ...
          cssWithDirection = {
            top: {
              borderColor:
                params.css.backgroundColor +
                ["", transparent, transparent, transparent].join(" "),
              borderStyle: [solid, dashed, dashed, dashed].join(" "),
              top: -1,
            },
            bottom: {
        ...
        return $("<" + beforeOrAfter + "></" + beforeOrAfter + ">").css(
          cssFinalUsed
        );
      };
      ...
      this.remind
        .css({
          ...
          marginLeft:
            $(this).outerWidth() * 0.5 -
            (this.remind.outerWidth() * parseInt(align)) / 100,
        })
        .prepend(
          $("<div></div>")
            .css(cssOuterLimit)
            .append(fnCreateCorner("before"))
            .append(fnCreateCorner("after"))
        );
    ...
    html5Validate: function (callback, options) {
      ...
      $(this).bind("submit", function () {
        if (
          $.html5Validate.isAllpass(elements, params) &&
          params.validate() &&
          $.isFunction(callback)
        ) {
          callback.call(this);
